...
max_retries_per_doc: 5            # OPTIONAL EsWriteConfig tuning (PER-DOCUMENT retries for rows ES rejects with a 429: es_rejected_execution_exception, write queue full); omit for connector default (3). 0 disables per-doc retries. Distinct from transport_max_retries (whole request): a 429'd document is only ever retried by this knob
...
- **Job parameters** are `--params` values applied at **run** time, overridable per run without
  ...
  - `pipeline_mode` (`batch` | `streaming`), `filter_condition` (a Spark SQL predicate), and the
    ...
    `transport_max_retries`, `max_retries_per_doc`, `require_existing_index`, `verify_certs` (each an
    optional config key; see
  ...
  - `request_timeout` (a positive integer, **seconds**) and
    ...
    re-sends the *whole* request on such a failure. Both apply to **both** modes.
  - `max_retries_per_doc` (a non-negative integer; `0` disables) is a **different** retry layer:
    how many times the connector re-sends an *individual document* that Elasticsearch rejects with a
    **429** (`es_rejected_execution_exception`, the ES write queue is full). The `_bulk` call returns
    HTTP 200 with the rejected row reported inside it, so `transport_max_retries` never sees a 429'd
