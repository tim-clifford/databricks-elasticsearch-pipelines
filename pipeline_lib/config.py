...
def require_pipeline_mode(value: object, where: str = "pipeline_mode") -> str:
    """A pipeline mode, restricted to an allow-list (batch|streaming). No default: absent/unknown fails.
    ...
    `--params pipeline_mode=...` must fail closed, not silently run the wrong mode). Both contexts accept
    the same two export modes; clearing a checkpoint is the dedicated `_checkpoint clear` job's job, not a
    pipeline_mode (see _VALID_PIPELINE_MODES)."""
